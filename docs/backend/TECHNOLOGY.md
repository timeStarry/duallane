...
The selections below describe the active Go backend under `apps/backend`.
Compose and the gateway select Go P2P, Workspace, worker and migration owners.
The Node online runtime is retired; its frozen compatibility evidence and
explicit offline storage tools remain. A passing local check still does not
authorize a release: the live version and release gates are recorded in
...
- `Dockerfile.p2p` and `Dockerfile.workspace` build the online Go services
  selected by Compose. Their default base images are digest-pinned; the
  Workspace recipe pins its libvips package inputs. Record the actual image
  IDs and build inputs for every release: source pins alone do not establish
  reproducible output or a successful deployment. See
  [Runtime and operations](OPERATIONS.md#3-images).
...
| Image processing | `github.com/davidbyttow/govips/v2/vips` and libvips | Workspace adapter; retain the compatibility gates in Section 5 |
...
can all change a digest. Echo requirement fixtures are frozen outputs of the
last retained Node implementation; their provenance and hashes are checked by
`node --test .github/tests/frozen-node-artifacts.test.mjs`. Do not regenerate
them from Go to conceal a mismatch. The Go tests compare both hashes and
PostgreSQL replay without additional audit/event writes. Deliberate contract
changes require a separately reviewed fixture/version transition.
...
## 5. Image Processing Decision

The Workspace image uses govips and libvips: the checked-in
...
the media runtime. Production ownership is recorded in
[Evolution](EVOLUTION.md), separately from each release's image validation.

Changes to the Go media adapter or native image must retain compatibility with
the frozen historical Sharp/Node behavior for:
...
The gate uses synthetic fixtures and golden metadata, not imported emote
...
Avatar and custom-emote processing are owned by Go. The retired Node/Sharp
runtime is not a fallback service; retain immutable historical oracle outputs
and the known-good Go image for compatibility and rollback. Local media
