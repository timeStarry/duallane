...
## 28. Personal Automatic Message Hiding (0.17)

Persistence and compatibility: [Chat preferences](WORKSPACE_CHAT_PREFERENCES.md).

- Personal settings -> Chat -> Message display provides “在聊天中自动隐藏消息”.
  The master switch defaults off. Its image/emote/long-message multi-select is
  rendered only while enabled, using the existing emote-pack choice pattern.
- First enablement selects all three categories; any subset, including empty,
  is allowed. Disabling preserves the selected categories. Saving failures
  restore the prior choice and show a safe warning.
- The preference belongs to the authenticated user and applies to ordinary
  Workspace chat and topic message bodies, including already loaded history.
  It is not a message deletion, access restriction, or a privacy filter for
  previews, reply quotations, search, pins, cards, notifications or exports.
  P2P has no account preference and remains unchanged by this setting.
- A matching message body is replaced with a compact category explanation and
  “展开消息” action. Reveal/collapse is local to the mounted message, not a
  persisted message-hide operation. Existing explicit manual hides still win.
  Content children and media requests are deferred until reveal.
- Images include available previewable image attachments and rendered HTTP(S)
  Markdown images. Emotes include image-emote tokens, structured emoji and
  Unicode emoji in displayed text; code examples are not interpreted as emotes
  or images. Mixed messages containing a selected category hide their body.
- Long messages use the existing boundary: more than 700 Unicode code points
  or more than 10 lines. Existing long-text preview behavior is unchanged while
  automatic hiding is off.
- Changes emit the existing actor-scoped `emote.settings.updated` event so an
  open chat can refresh preferences. Reload also fetches the authoritative value.

## 29. IME Confirmation And Topic Actions (0.17)

- Enter handled by an input method must not trigger send or mention selection.
  Guard before both parent capture handlers and editor handlers, preserving the
  native commit action; the next ordinary Enter sends normally and Shift+Enter
  remains a newline. Apply the same rule to the private-lane textarea.
- Check both `isComposing` and legacy IME `keyCode === 229`: WebKit can dispatch
  composition end before the confirming keydown. See the
  [MDN keydown event guidance](https://developer.mozilla.org/en-US/docs/Web/API/Element/keydown_event)
  and [WebKit issue 165004](https://bugs.webkit.org/show_bug.cgi?id=165004).
- Topic message reply/sync actions use the existing Lucide icon family with
  accessible names, tooltips, visible pressed state, keyboard focus and 44px
  narrow-screen targets. The composer's “同步到群聊” checkbox keeps its label.
