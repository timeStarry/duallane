...
import { createWorkspaceStorageObjectRegistry } from "./storage-registry.mjs";
import { workspaceContentObjectKey } from "./storage-paths.mjs";
...
function dedupeError(code, record) {
  const error = new Error(`Workspace storage dedupe failed: ${code}`);
  error.code = code;
...
function safeErrorCode(error) {
  ...
  return /^[A-Za-z0-9_.-]{1,64}$/.test(code) ? code : "storage.dedupe_failed";
}
