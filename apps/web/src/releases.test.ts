...
describe("release history", () => {
  it("is unique, complete, and sorted newest first", () => {
    ...
    expect(DUAL_LANE_RELEASES.map((release) => release.version)).toEqual(["0.15.3", "0.15.2", "0.15.1", "0.15.0", "0.14.3", "0.14.2", "0.14.1", "0.14.0", "0.13.2", "0.13.1", "0.13.0", "0.12.0", "0.11.0", "0.10.0", "0.9.0", "0.8.0", "0.7.0", "0.6.0", "0.5.0", "0.4.0", "0.3.0", "0.2.0", "0.1.0"]);
  ...
  it("matches both published package versions", () => {
    expect(rootPackage.version).toBe("0.15.3");
    expect(webPackage.version).toBe("0.15.3");
    expect(DUAL_LANE_RELEASES[0].version).toBe(rootPackage.version);
    expect(DUAL_LANE_RELEASES[0].version).toBe(webPackage.version);
  });

  it("describes the 0.15.3 user-facing scope and release date", () => {
    const latest = DUAL_LANE_RELEASES[0];
    const publicCopy = JSON.stringify(latest);
    expect(latest.version).toBe("0.15.3");
    expect(latest.releasedAt).toBe("2026-08-21");
    expect(publicCopy).toContain("通知");
    expect(publicCopy).toContain("图片");
    expect(publicCopy).toContain("话题");
    expect(publicCopy).toContain("移动端");
