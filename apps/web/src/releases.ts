...
export const DUAL_LANE_RELEASES: DualLaneRelease[] = [
  {
    ...
    title: "更完整的表情库与个人空间",
    summary: "个人设置、消息时间线和表情收藏进一步完善，日常操作更清楚也更顺手。",
    categories: [
      { title: "表情", items: ["个人表情库改为图片优先的网格管理，支持短名称、批量上传和合集", "单张表情与合集可以混合排序，合集也可分享给空间成员", "Emoji 会记住近期常用内容，表情面板会保留上次选择的页签"] },
      { title: "消息", items: ["可隐藏任意消息，连续隐藏的内容会合并显示并可随时恢复", "表情回复和常用操作更易触达，右键消息也可打开完整操作菜单", "消息中的分割线改为随内容宽度变化的虚线样式"] },
      { title: "体验", items: ["个人设置使用清楚的分层导航，聊天、文件和成员侧栏会展示与当前任务相关的内容", "邀请历史支持独立滚动，群头像选择和全局滚动条样式更加协调", "网页版本落后时会提示刷新，减少更新后继续使用旧页面的问题"] }
    ]
  },
  {
    version: "0.10.0",
