...
describe("emote rendering", () => {
  ...
  it("renders whitelisted platform tokens and leaves unknown tokens as text", () => {
    const rendered = renderMessageParts("收到 [bili:like] [qq:微笑] [douyin:laughwithtears] [feishu:OK]");
    ...
    expect(parts[4]).toBe(" ");
    expect(parts[5]).toHaveProperty("props.token", "[douyin:laughwithtears]");
    expect(parts[6]).toBe(" ");
    expect(parts[7]).toHaveProperty("props.token", "[feishu:OK]");
  ...
  it("hides placeholder-only packs from the picker while preserving message compatibility", () => {
