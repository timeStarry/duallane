...
func (s *MobilePGStore) Exchange(ctx context.Context, hash, verifier, redirect string, tokens MobileTokens, now time.Time) error {
	...
	if uri != redirect || !verifyPKCE(verifier, challenge) {
		return errMobileInvalid
	}
	family, err := newUUID()
	if err != nil {
...
func (s *MobilePGStore) RecordRejection(ctx context.Context, operation string, meta RequestMeta, now time.Time) error {
	id, err := newUUID()
	if err != nil {
		return err
	}
	...
	_, err = s.pool.Exec(ctx, `INSERT INTO audit_logs
		(id,space_id,action,target_type,target_id,result,reason,ip_address,user_agent,request_id,created_at)
		VALUES($1,$2,'auth.mobile.rejected','mobile_auth',$3,'rejected','auth.mobile_invalid',$4,$5,$6,$7)`,
		id, DefaultSpaceID, operation, meta.IPAddress, meta.UserAgent, meta.RequestID, now)
	return err
}
