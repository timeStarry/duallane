		...
		for _, passive := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				cfg := config.WorkspaceConfig{
					DataDir: dataDir, StorageDriver: provider, WorkerValidateOnly: passive,
				...
				store, err := newMaintenanceBlobStoreWithObservation(ctx, cfg, storage.ObservationOptions{Observer: recorder, Service: platformmetrics.ServiceWorker})
				if err != nil {
					t.Fatal(err)
				}
				...
				expected := `duallane_workspace_object_operations_total{operation="open",outcome="failure",service="worker"} 1`
