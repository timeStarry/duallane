...
test("space owner broadcasts one detailed mobile release guide through Echo", async ({ page }) => {
  ...
  await composer.fill("/release 0.19.2");
  ...
  await expect(interaction.getByText("v0.19.2 更新已发布", { exact: true })).toBeVisible();
  const card = chat.getByRole("group", { name: /DualLane v0\.19\.2 版本更新/ });
  await expect(card).toBeVisible();
  await expect(card.getByText("发送后回到最新消息", { exact: true })).toBeVisible();
  await expect(card.getByText("阅读历史时主动发送消息，会回到最新位置，立即看到刚发出的内容。", { exact: true })).toBeVisible();
  await expect(card.getByText("聊天或话题 -> 输入消息 -> 发送", { exact: true })).toBeVisible();
  ...
  await composer.fill("/release v0.19.2");
  await chat.locator("form.workspace-composer button.workspace-send-button").click();
  await expect(interaction.getByText("v0.19.2 更新已发布", { exact: true })).toBeVisible();
  await expect(chat.getByRole("group", { name: /DualLane v0\.19\.2 版本更新/ })).toHaveCount(1);
});
